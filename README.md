...
**Google sign-in works in the packaged apps too.** A native shell has no web origin for a provider to return to, so it claims the `libreta://` URL scheme instead (`src-tauri/tauri.conf.json` → `plugins.deep-link`): sign-in opens the *system browser*, Google redirects to `libreta://auth-callback`, the OS wakes the app, and `sessionFromCallback()` turns it into a session — handling both the implicit flow (tokens in the fragment) and PKCE (a code in the query). On Windows and Linux the OS delivers that by relaunching the binary with the URL as an argument, so `tauri-plugin-single-instance` forwards it to the window already open. Password resets email a link to the hosted web app, where the user sets a new password and then signs in here with it. Only magic links stay web-only — the mail client opens them in a browser, which can only sign you in *there*.

Two values in `js/cloud/config.js` (`libreta://auth-callback` and `LIBRETA_WEB_URL`) must also be listed in Supabase → Authentication → URL Configuration, or the provider refuses to redirect.
