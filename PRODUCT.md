...
**Last updated:** June 22, 2026 · **Status:** Closed test shipped (Jun 20), now in Week 2 feedback triage.
...
| **Hosting** | Vercel (static deploy via `./build.sh` → `dist/`) |
...
css/                Numbered stylesheets (01-tokens through 22-filters)
js/
  core/             Plumbing
...
6. **Cache-bust on every deploy.** Bump `?v=N` in `index.html` after any CSS/JS change. Production users will otherwise get stale cached assets.
