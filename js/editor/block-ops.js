...
/* Enter at the start of a block → insert an empty paragraph directly above it,
   leaving this block (and the caret) untouched. */
function insertEmptyBlockAbove(id){
  const loc=locate(id); if(!loc) return;
  const nb=mkBlock('paragraph','');
  loc.arr.splice(loc.idx,0,nb);
  const row=document.querySelector(`.bk-row[data-id="${id}"]`);
  if(row) row.before(mkBkEl(nb));
  updNums(); sched();
}
