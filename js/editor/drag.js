...
function dropZone(e,row){
  ...
  // Column-creation lives only in a slim strip at each edge; the whole rest of the row
  // reads as a between-blocks insert (top/bottom). Heavily favours the common case —
  // a stray column is one ⌘Z away, a missed between-drop is annoying.
  const w=right-left, edge=Math.min(28,w*0.08);
