...
function renderEditorIcon(doc){
  if(S.peekOpen) return renderPeekIcon(doc);
...
/* Icon in the side-peek (distinct element ids from the editor so both can coexist). */
function renderPeekIcon(doc){
  const area=document.getElementById('peek-icon-area'); if(!area) return;
  const addBtn=document.getElementById('peek-icon-add-btn');
  const icon=doc&&doc.meta&&doc.meta.icon;
  if(icon){
    const isImg=isBlobRef(icon)||icon.startsWith('data:')||icon.startsWith('http');
    area.innerHTML=`<div class="icon-display${isImg?' has-img':''}" onclick="openIconPicker(event,this)" title="Change icon">${iconInner(icon,'1em')}</div>`;
    if(addBtn) addBtn.style.display='none';
  } else { area.innerHTML=''; if(addBtn) addBtn.style.display='inline-flex'; }
}
...
function renderIconPickerContent(tab){
  ...
  const tabs=`<div class="ip-tabs">
    ...
    <button class="ip-tab${tab==='icons'?' on':''}" onclick="renderIconPickerContent('icons')">✦ Icons</button>
  ...
  if(tab==='emoji'){
    ...
    const cur=S.iconColor||'';
    const swatches=LINE_ICON_COLORS.map(c=>
      `<button class="ip-sw${c===cur?' on':''}${c?'':' ip-sw-def'}" title="${c||'Default'}" style="${c?`--sw:${c}`:''}" onclick="setIconColor('${c}')"></button>`
    ).join('');
    const rows=Object.entries(LINE_ICON_CATS).map(([cat,names])=>
      `<div class="ip-cat">${cat}</div><div class="ip-grid ip-grid-li">${names.map(n=>
        `<button class="ip-li" data-n="${n}" onclick="applyLineIcon('${n}')">${lineIconSvg(n,cur,'22px')}</button>`
      ).join('')}</div>`
    ).join('');
    body=`<div class="ip-scroll">
      <div class="ip-cat">Colour</div><div class="ip-swatches">${swatches}</div>
      ${rows}
      <button class="ip-clr" onclick="applyIcon('')">Remove icon</button>
    </div>`;
  } else {
...
/* Recolour the line-icon palette in place (just re-renders the grid; doesn't apply). */
function setIconColor(c){ S.iconColor=c||''; renderIconPickerContent('icons'); }
/* Commit a line icon with the currently-selected colour → stored as li:<name>[:#hex]. */
function applyLineIcon(name){ const c=S.iconColor||''; applyIcon('li:'+name+(c?':'+c:'')); }
