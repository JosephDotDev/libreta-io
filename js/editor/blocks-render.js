...
function mkBkEl(blk){
  ...
  if(blk.type==='divider'){
    ...
    if(blk.src){
      ...
      row.innerHTML=gutter+`<div class="bk-wrap"><div class="bk-img-wrap"${wStyle}><img src="${srcFor(blk.src)}" alt="${blk.caption||''}">${handles}<div class="bk-img-overlay"><button class="bk-img-btn" onclick="toggleImgCaption('${blk.id}')">${blk.hideCaption?'Show caption':'Hide caption'}</button><button class="bk-img-btn" onclick="replaceBlkImg('${blk.id}')">Replace</button><button class="bk-img-btn" onclick="delBlk('${blk.id}')">Remove</button></div></div>${capHtml}</div>`;
    } else {
