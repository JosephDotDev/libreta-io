...
function mkBkEl(blk){
  ...
  if(blk.type==='divider'){
    ...
    const ico=blk.icon||'💡';
    row.innerHTML=gutter+`<div class="bk-wrap"><div class="bk-callout"><button class="bk-callout-ico" onclick="openCalloutIconPicker(event,this,'${blk.id}')" title="Pick an icon">${calloutIconInner(ico)}</button><div class="bk" contenteditable="true" data-t="callout" data-id="${blk.id}" data-ph="Type a note…" spellcheck="true">${blk.content}</div></div></div>`;
    ...
    if(blk.src){
      ...
      row.innerHTML=gutter+`<div class="bk-wrap"><div class="bk-img-wrap"><img src="${srcFor(blk.src)}" alt="${blk.caption||''}"><div class="bk-img-overlay"><button class="bk-img-btn" onclick="toggleImgCaption('${blk.id}')">${blk.hideCaption?'Show caption':'Hide caption'}</button><button class="bk-img-btn" onclick="replaceBlkImg('${blk.id}')">Replace</button><button class="bk-img-btn" onclick="delBlk('${blk.id}')">Remove</button></div></div>${capHtml}</div>`;
    } else {
