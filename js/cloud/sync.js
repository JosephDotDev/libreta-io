/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ---------------------------------------------------------------------------
   OPTIONAL sync. Libreta works fully signed out — this layer never gates the
   app and never blocks boot. Signed in, it simply:
     • keeps a copy of the workspace in the user's Supabase account,
...
const Cloud = (()=>{
  ...
  /* Cloud sync stands down while the notes live in a folder: that folder IS the
     user's sync, and two systems writing the same pages would fight. */
  function folderMode(){ try{ return typeof Workspace!=='undefined' && Workspace.active; }catch(e){ return false; } }
  function client(){
    if(sb) return sb;
  ...
  /* ── boot: called by init.js after DB.load(). NEVER gates the app and never
        blocks on a signed-out user — Libreta is perfectly usable with no account.
        If a session is already stored, adopt it and start syncing. */
  async function boot(){
    if(folderMode() || !client()) return;
    ...
    catch(e){ console.warn('[cloud] could not read the session',e); return; }
    // The recovery session is loaded by now, so strip the token from the address
    // bar (a refresh shouldn't replay it) without losing the session.
    if(recovery) cleanRecoveryUrl();
    if(recovery && session){
      // Password-recovery link: make the user set a new password first, then
      // continue into the app on the same session.
      user = await showRecoveryGate();
    }else if(session){
      // Cover the screen while we pull, so the user never sees this device's
      // older copy flash before their synced data lands.
      user = session.user; showLoadingGate();
    }else{
      return;                    // signed out — nothing to do, the app runs locally
    }
    if(user) await startSync();
  }

  /* Turn a live session into a running sync. */
  async function startSync(){
    await pull();              // bring the cloud copy down
    purgeStaleMonolith();      // one-time: drop the obsolete state.json once on records mode
    installAutosync();         // watch for local changes to push back up
    installRealtime();         // cross-device push notifications
    startPoll();               // slow fallback poll for events Realtime missed
    mountStatusChip();
  }

  /* Settings → "Sign in to sync". Opens the account card on demand; resolves
     whether or not the user went through with it. */
  async function signIn(){
    if(folderMode()){
      toast('Your notes are in a folder — that folder is already your sync. Switch to this device first.');
      return false;
    }
    if(!client()){ toast('Sync is unavailable — the client could not be loaded'); return false; }
    const u = await showAuthGate(false);
    if(!u) return false;                     // cancelled
    user = u;
    await startSync();
    ...
    return new Promise(resolve=>{
      ...
      wrap.innerHTML=`
        <div class="ag-card ag-card-v2">
          <div class="ag-logo">Libre<span>ta</span></div>
          <h1 class="ag-promise">Sync across<br>your devices.</h1>
          <div class="ag-promise-sub">Your notes stay on this device and keep working offline. Signing in adds a copy in your account so your other devices can catch up.</div>
          <div class="ag-pills"><span class="ag-pill ag-pill-g">Still works offline</span><span class="ag-pill ag-pill-b">Optional, always</span></div>
          <div class="ag-syncdiv"><span>Sign in or create an account</span></div>
          ...
          <button id="ag-skip" type="button" class="ag-link ag-cancel">Not now</button>
        </div>`;
      document.body.appendChild(wrap);
      let mode='signin';
      ...
      $('#ag-skip').onclick=()=>{ wrap.remove(); resolve(null); };
      wrap.addEventListener('click', e=>{ if(e.target===wrap){ wrap.remove(); resolve(null); } });
      document.addEventListener('keydown', function esc(e){ if(e.key==='Escape' && document.body.contains(wrap)){ wrap.remove(); resolve(null); document.removeEventListener('keydown',esc); } });
  ...
  async function signOut(){ try{ if(sb) await sb.auth.signOut(); }catch(e){} location.reload(); }
  ...
  return { boot, signIn, startSync, push, syncNow, signOut, deleteEverything, reconcileRecords, planReconcile, localManifest, setSyncMode, setMediaSync, planMedia, needsMediaSync,
           get mediaSyncOn(){ return _mediaSyncOn(); }, get user(){ return user; }, get on(){ return !!user; }, get available(){ return !folderMode(); } };
})();
